...
class ProstNFoundMeta(nn.Module):
    ...
    def forward(self, data, include_postprocessed_heatmaps=False):
        ...
        if isinstance(self.model, ProstNFound):
            ...
            # Fixed 2026-09-05: this used to iterate `discrete_prompts` only,
            # never `floating_point_prompts`, even though
            # `Proside._build_sparse_embedding` handles both. So a cfg
            # setting `model_kw.floating_point_prompts: [age, psa, ...]`
            # silently trained identically to an empty prompt list -- never
            # actually bitten anyone since every existing cfg in this repo
            # sets `floating_point_prompts: []`. Found while wiring up a
            # prompts-on comparison run.
            prompts = {}
        ...
        if isinstance(self.model, SETR):
            ...
            for name, param in self.model.named_parameters():
                if "model.image_encoder" in name: # the pretrained dino backbone inside Proside
                    encoder_parameters.append(param)
                else: # feature_proj, mask_decoder, class_decoder, etc.
                    warmup_parameters.append(param)
            ...
            logging.info(
                f"Encoder Params: {encoder_params:,} | Trainable (all_others): {trainable:,} | "
