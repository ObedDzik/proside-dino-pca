...
class MaskDecoder(nn.Module):
    ...
    #original forward for the mask decoder
    def forward(
        self,
    ...
    #code for restricting mask decoding
    # def forward(
    #     self,
    #     image_embeddings: torch.Tensor,
    #     image_pe: torch.Tensor,
    ...
    #     Arguments:
    #     ...
    #     key_padding_mask (torch.Tensor, optional): (B, H*W) bool, True =
    #         needle-region location the mask/iou tokens are allowed to gather
    #         from. If None, tokens attend over the full grid unrestricted.
    #     """
    #     masks, iou_pred, *out = self.predict_masks(
    #         image_embeddings=image_embeddings,
    #         image_pe=image_pe,
    ...
    #     # Run the transformer -- restricted, if key_padding_mask given, so the
    #     # mask/iou tokens can only gather a summary from needle-region locations;
    #     # the dense spatial grid itself (src) stays full-resolution for upscaling
    #     # print(f"THE KPM IS WORKING PROPERLY:shape: {key_padding_mask.shape}, mean: {key_padding_mask.sum()}")
    #     hs, src = self.transformer(src, pos_src, tokens, key_padding_mask=key_padding_mask)
    #     iou_token_out = hs[:, 0, :]
    ...
    #original codes for the mask decoder
    def predict_masks(
        self,
        image_embeddings: torch.Tensor,
        image_pe: torch.Tensor,
...
#original class decoder code, attends to all the image not restricted
...
#using this for needle region only decoding
class ClassDecoder(nn.Module):
    def __init__(
        self,
        *,
